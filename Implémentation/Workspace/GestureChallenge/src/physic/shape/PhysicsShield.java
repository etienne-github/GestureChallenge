...
public class PhysicsShield extends MTTriangleMesh implements IPhysicsComponent {
...
//    		Vertex[] triangulatedBodyVerts = physicsTris.toArray(new Vertex[physicsTris.size()]);
    		...
    		this.bodyDefB4CreationCallback(dymBodyDef);
    		this.body = world.createBody(dymBodyDef);
    		for (int i1 = 0; i1 < triangulatedBodyVerts.length/3; i1++) {
    			...
    			Vertex vertex1 = triangulatedBodyVerts[i1*3];
    			Vertex vertex2 = triangulatedBodyVerts[i1*3+1];
    			Vertex vertex3 = triangulatedBodyVerts[i1*3+2];
