 ...
 *
 ***********************************************************************/
package popup.touch.gestureProcessor;
 ...
 */
public class TapAndHoldCountProcessor extends AbstractCursorProcessor implements IPreDrawAction{
	 ...
	 * @param pa the pa
	 */
	public TapAndHoldCountProcessor(AbstractMTApplication pa) {
		this(pa, 1800, false);
	}
	
	public TapAndHoldCountProcessor(AbstractMTApplication pa, int duration){
	 ...
	 */
	public TapAndHoldCountProcessor(AbstractMTApplication pa, int duration, boolean stopPropatation) {
		...
		if (locked.size() >=1){
			...
			if (getFreeComponentCursors().size() >= 1){ //Only start if this is the first cursor on the component
				if (this.getLock(c)){//See if we can obtain a lock on this cursor (depends on the priority)
					...
					int cursorNumber = getFreeComponentCursors().size();
					this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_STARTED, positionEvent.getCurrentTarget(), c, false, c.getPosition(), this.holdTime, 0, 0,cursorNumber));
					try {
...
//			IMTComponent3D currentTarget = c.getCurrentEvent().getCurrentTarget(); //FIXME this will often return the wrong target since we are not in a processInputEvent() method!
			...
			if (elapsedTime >= holdTime){
				...
				if ( (intersection != null || comp instanceof MTCanvas) //hack - at canvas no intersection..
						&& 
					Vector3D.distance2D(buttonDownScreenPos, screenPos) <= this.maxFingerUpDist*2
				){
					this.unLock(c);
					int cursorNumber = getFreeComponentCursors().size();
...
//						app.unregisterPre(this);
					...
					this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_ENDED, currentTarget, c, true, screenPos, this.holdTime, elapsedTime, normalized,cursorNumber));
					...
					this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_ENDED, currentTarget, c, false, screenPos, this.holdTime, elapsedTime, normalized,cursorNumber));
					//TODO here
				}
				this.unLock(c); 
				try {
//					app.unregisterPre(this);
				...
				int cursorNumber = getFreeComponentCursors().size();
				this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_UPDATED, currentTarget, c, false, screenPos, this.holdTime, elapsedTime, normalized,cursorNumber));
			}
		}
	}
	...
	public void cursorUpdated(InputCursor c, AbstractCursorInputEvt positionEvent) {
		...
		if (locked.contains(c)){
			...
			//logger.debug("Distance between buttondownScreenPos: " + buttonDownScreenPos + " and upScrPos: " + buttonUpScreenPos +  " is: " + Vector3D.distance(buttonDownScreenPos, buttonUpScreenPos));
			if (Vector3D.distance2D(buttonDownScreenPos, screenPos) > this.maxFingerUpDist*2f){
				logger.debug("DISTANCE TOO FAR OR NO INTERSECTION");
				this.unLock(c); 
				try {
//					app.unregisterPre(this);
				...
				int cursorNumber = getFreeComponentCursors().size();
				this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_ENDED, positionEvent.getCurrentTarget(), c, false, screenPos, this.holdTime, elapsedTime, normalized,cursorNumber));
		...
		if (locked.contains(c)){
			...
			if (free.size() > 0){ 			//check if there are other cursors on the component, we could use 
				...
				{ 	//Check if we have the priority to use this other cursor and if cursor is in range
					...
					int cursorNumber = getFreeComponentCursors().size();
					this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_ENDED, positionEvent.getCurrentTarget(), c, false,  c.getPosition(), this.holdTime, elapsedTime, normalized,cursorNumber));
					try {
//						app.unregisterPre(this);
						app.unregisterPreDrawAction(this);
					} catch (Exception e) {
...
//					app.unregisterPre(this);
	...
	public void cursorLocked(InputCursor c, IInputProcessor lockingAnalyzer) {
		...
		float normalized = (float)elapsedTime / (float)this.holdTime;
		int cursorNumber = getFreeComponentCursors().size();
		this.fireGestureEvent(new TapAndHoldCountEvent(this, MTGestureEvent.GESTURE_ENDED, c.getCurrentEvent().getCurrentTarget(), c, false, c.getPosition(), this.holdTime, elapsedTime, normalized,cursorNumber));
